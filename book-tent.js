/* =========================================
   TENT BOOKING PAGE — book-tent.js
   Reads ?tent=X from URL, populates the
...
const TENTS = {
  1: {
    name:     'Menzah Bellota — Tent 1',
    subtitle: 'Forest View Tent · Canopy of Oaks',
    img:      'images/best seller.webp',
    beds:     '🛏️  1 Double Bed',
    guests:   '👥  Max 2 Guests',
  },
  2: {
    name:     'Menzah Bellota — Tent 2',
    subtitle: 'Star Gazer Tent · Private Night Sky',
    img:      'images/best seller.webp',
    beds:     '🛏️  1 Double Bed',
    guests:   '👥  Max 2 Guests',
  },
  3: {
    name:     'Menzah Bellota — Tent 3',
    subtitle: 'Twin Tent · Perfect for Friends',
    img:      'images/best seller.webp',
    beds:     '🛏️  2 Single Beds',
    guests:   '👥  Max 2 Guests',
  },
  4: {
    name:     'Menzah Bellota — Tent 4',
    subtitle: 'Family Tent · Summit Panorama',
    img:      'images/best seller.webp',
    beds:     '🛏️  1 Double + 2 Single',
    guests:   '👥  Max 4 Guests',
  },
};

/* ---- 1. Read ?tent= param & populate summary ---- */
function loadTent() {
  const params  = new URLSearchParams(window.location.search);
  const tentId  = parseInt(params.get('tent')) || 1;
  const tent    = TENTS[tentId] || TENTS[1];

  document.getElementById('summaryImg').src              = tent.img;
  document.getElementById('summaryImg').alt              = tent.name;
  document.getElementById('summaryRoomNum').textContent  = String(tentId).padStart(2, '0');
  document.getElementById('summaryName').textContent     = tent.name;
  document.getElementById('summarySubtitle').textContent = tent.subtitle;
  document.getElementById('summaryBeds').textContent     = tent.beds;
  document.getElementById('summaryGuests').textContent   = tent.guests;
  document.getElementById('hiddenRoom').value            = tent.name;
  document.title = `Book ${tent.name} — Menzah Bellota`;
}
...
function setupForm() {
  ...
  form.addEventListener('submit', function (e) {
    ...
    const tentName = document.getElementById('hiddenRoom').value;
